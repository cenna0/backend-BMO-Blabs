...
| Login/session state | PostgreSQL/secure session store | Backend | Backend/session expiry | Mobile stores only session material needed by client |
...
| User/device settings | PostgreSQL via Backend | Backend, mobile, Hermes context adapter | User/Backend | Voice settings are user-editable within safe catalog |
...
| Hardware protocol | HW Contract v1.0.5 | Firmware/backend | Contract governance | P9 proposal may be additive vNext only |
