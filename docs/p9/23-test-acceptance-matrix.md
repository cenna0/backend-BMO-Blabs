...
|---|---|---|
| Backend tests | Node 22.23.1: 46 files passed, 1 skipped; 226 tests passed, 1 skipped DB HTTP suite. Slice 2B account/profile/recovery/avatar/personalization unit/route/storage coverage is included | Disposable-Postgres authenticated HTTP acceptance remains separately opt-in |
...
| Auth/recovery | Slice 2B source/tests cover self-service/optional legacy invitation, exact DOB, DOB-free canonical `SafeUser`, enumeration parity, independent limits, hash-only 600-second token, atomic reuse rejection, Argon2id replacement and revoke-all | Disposable-Postgres route/concurrency acceptance remains an authorized candidate gate |
| Profile/avatar/personalization | Slice 2B source/tests cover normalized ownership/conflict, strict defaults/patch/tenant binding, multipart bounds, signature/decode validation, WebP transcode, opaque path, cleanup and hardened media response | Persistent-volume/runtime acceptance remains gated; personalization-to-Hermes context is a later slice |
