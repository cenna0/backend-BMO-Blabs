# BMO Piper-Only Production Cutover and Legacy Audio Purge Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Safely promote the already-built Piper-only Audio Service, verify the complete BMO voice path, then remove Kokoro/RVC runtime artifacts without losing rollback capability or touching unrelated P9/Spotify/WhatsApp work.
...
- Repository: `/opt/bmo/app`, branch `main`, current source `4fcd1cb`.
...
- Existing candidate images exist and must be identity-checked before use:
  - Audio: `bmo-audio@sha256:24e1c4244ea8868f731d819ea75cb57c1e464b6df3bd9679d65fd4711643488c`.
  - Backend: `bmo-p9.1-candidate@sha256:71011f1b2ee10852774bddd1bd03dd6ae0f7a66eea1e190e1c4981023792efe0`.
- Current production Audio Service remains `bmo-audio@sha256:62ad9adead83d863ab2bf28a2ac75e5a116dc68bab8ff06eec81b7a0407ddb34`, healthy on port `8001`.
...
- **BLOCKED_RESOURCE:** If `MemAvailable < 4 GiB` before candidate start, do not start candidate or purge anything. A user-authorized maintenance override may stop only `bmo-production-audio-1` after its digest/state are recorded, solely to free memory for the isolated candidate. If the candidate fails, restore the old Audio image immediately.
...
- **Rollback target:** restore only the recorded old Audio image and old Audio env, recreate only `bmo-production-audio-1`, then rerun Audio and backend health checks. Do not recreate Backend, PostgreSQL, Hermes, Caddy, WhatsApp, or monitoring.
...
**Files:**
- Read: `/opt/bmo/app` Git state, Compose labels, Docker state, resource state.
- Create: `/opt/bmo/temp/piper-only-cutover-evidence/<UTC>/`.
...
```bash
mkdir -p /opt/bmo/temp/piper-only-cutover-evidence/$(date -u +%Y%m%dT%H%M%SZ)
EVIDENCE_DIR=$(printf '%s\\n' /opt/bmo/temp/piper-only-cutover-evidence/* | sort | awk 'END{print}')
docker inspect bmo-production-audio-1 --format 'name={{.Name}} image={{.Config.Image}} image_id={{.Image}} status={{.State.Status}} health={{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}} restarts={{.RestartCount}} oom={{.State.OOMKilled}}' | tee "$EVIDENCE_DIR/production-audio-before.txt"
docker inspect bmo-production-p9-backend-1 --format 'name={{.Name}} image={{.Config.Image}} image_id={{.Image}} status={{.State.Status}} health={{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}} restarts={{.RestartCount}} oom={{.State.OOMKilled}}' | tee "$EVIDENCE_DIR/production-backend-before.txt"
docker inspect bmo-production-p9-postgres-1 --format 'name={{.Name}} image={{.Config.Image}} image_id={{.Image}} status={{.State.Status}} health={{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}} restarts={{.RestartCount}} oom={{.State.OOMKilled}}' | tee "$EVIDENCE_DIR/production-postgres-before.txt"
docker inspect bmo-production-audio-1 --format '{{json .Config.Labels}}' > "$EVIDENCE_DIR/audio-labels.json"
```
...
```bash
{
  ...
  df -h /opt/bmo
  du -sb /opt/bmo/models /opt/bmo/cache/audio /opt/bmo/temp /opt/bmo/archive 2>/dev/null
...
docker image inspect bmo-audio@sha256:62ad9adead83d863ab2bf28a2ac75e5a116dc68bab8ff06eec81b7a0407ddb34 --format 'id={{.Id}} size={{.Size}} repo_digests={{json .RepoDigests}}' > "$EVIDENCE_DIR/rollback/old-audio-image.txt"
sha256sum /opt/bmo/app/docker-compose.yml /opt/bmo/config/audio.env > "$EVIDENCE_DIR/rollback/config-sha256.txt"
stat -c '%n mode=%a owner=%U:%G size=%s' /opt/bmo/config/audio.env >> "$EVIDENCE_DIR/rollback/config-sha256.txt"
```
...
```bash
cd /opt/bmo/app
docker image inspect bmo-audio@sha256:24e1c4244ea8868f731d819ea75cb57c1e464b6df3bd9679d65fd4711643488c --format 'id={{.Id}} size={{.Size}} revision={{index .Config.Labels "org.opencontainers.image.revision"}}'
docker image inspect bmo-p9.1-candidate@sha256:71011f1b2ee10852774bddd1bd03dd6ae0f7a66eea1e190e1c4981023792efe0 --format 'id={{.Id}} size={{.Size}} revision={{index .Config.Labels "org.opencontainers.image.revision"}}'
...
```bash
docker run --rm --network none bmo-audio@sha256:24e1c4244ea8868f731d819ea75cb57c1e464b6df3bd9679d65fd4711643488c python -c 'import importlib.util; required=("faster_whisper","piper","onnxruntime"); forbidden=("kokoro","spacy","torch","transformers"); assert all(importlib.util.find_spec(x) for x in required); assert not any(importlib.util.find_spec(x) for x in forbidden); print("dependency-contract=ok")'
docker run --rm --network none bmo-audio@sha256:24e1c4244ea8868f731d819ea75cb57c1e464b6df3bd9679d65fd4711643488c sh -lc 'command -v ffmpeg && command -v ffprobe && ! command -v rvc'
```
...
```bash
CANDIDATE_ENV=/opt/bmo/config/piper-only-candidate.env
umask 077
python3 - "$CANDIDATE_ENV" <<'PY'
import os, re, sys
from pathlib import Path
src = Path('/opt/bmo/config/audio.env')
dst = Path(sys.argv[1])
allow = {
...
values.update({
    ...
    'PIPER_SPEAKER_ID':'0', 'PIPER_MANIFEST_PATH':'/opt/bmo/models/piper/PIPER_ASSET_MANIFEST.json',
    'RUNTIME_MODELS_ROOT':'/opt/bmo/models/runtime',
    'MODEL_MANIFEST_PATH':'/opt/bmo/models/runtime/MODEL_MANIFEST.json',
    'TTS_TEMP_DIR':'/opt/bmo/temp/tts-candidate',
})
...
Expected: candidate env is mode `600`, contains no legacy key, and is used by setting `AUDIO_ENV_FILE=/opt/bmo/config/piper-only-candidate.env`. If the candidate overlay still injects the production env file, fix the overlay to consume `AUDIO_ENV_FILE` before proceeding.
...
**Files:**
- Runtime only: candidate project `bmo-piper-only-candidate`.
...
If blocked and no explicit maintenance authorization exists, stop this plan at Task 3. With the current user authorization, record the intentional maintenance window, stop only `bmo-production-audio-1` after saving its digest/config, leave Backend/PostgreSQL/Hermes/monitoring running, and re-evaluate the gate. If candidate verification fails, restart the old Audio image immediately before reporting the failure.
...
```bash
export P9_CANDIDATE_IMAGE=bmo-p9.1-candidate@sha256:71011f1b2ee10852774bddd1bd03dd6ae0f7a66eea1e190e1c4981023792efe0
export AUDIO_CANDIDATE_IMAGE=bmo-audio@sha256:24e1c4244ea8868f731d819ea75cb57c1e464b6df3bd9679d65fd4711643488c
export AUDIO_ENV_FILE=/opt/bmo/config/piper-only-candidate.env
cd /opt/bmo/app
docker compose --project-name bmo-piper-only-candidate --env-file /opt/bmo/config/p9.1/compose.env -f p9.1-compose.yml -f ops/deploy/piper-only-candidate-compose.yml config --format json > "$EVIDENCE_DIR/candidate-compose.json"
...
assert all(v.get('read_only') for v in a.get('volumes',[]) if v.get('target','').startswith(('/opt/bmo/models','/opt/bmo/cache/audio')))
...
```bash
cd /opt/bmo/app
P9_CANDIDATE_IMAGE=bmo-p9.1-candidate@sha256:71011f1b2ee10852774bddd1bd03dd6ae0f7a66eea1e190e1c4981023792efe0 \
AUDIO_CANDIDATE_IMAGE=bmo-audio@sha256:24e1c4244ea8868f731d819ea75cb57c1e464b6df3bd9679d65fd4711643488c \
AUDIO_ENV_FILE=/opt/bmo/config/piper-only-candidate.env \
PIPER_CANDIDATE_WAV=/opt/bmo/temp/p8-piper-production/public-speech.wav \
...
Promotion is allowed only when Tasks 1–4 are all green, candidate Audio and Backend image IDs match the recorded digests, production container identities are unchanged, and the complete candidate fake-ESP flow produced a valid non-empty MP3. Otherwise record `BMO_PIPER_ONLY_PURGE_RESULT=BLOCKED` and stop.

### Task 5: Promote only the Audio Service with deterministic rollback

**Files:**
- Modify only the protected runtime image reference used by `/opt/bmo/app/docker-compose.yml`.
...
```bash
old_audio_image=$(docker inspect bmo-production-audio-1 --format '{{.Config.Image}}')
old_backend_image=$(docker inspect bmo-production-p9-backend-1 --format '{{.Config.Image}}')
candidate_audio_image=bmo-audio@sha256:24e1c4244ea8868f731d819ea75cb57c1e464b6df3bd9679d65fd4711643488c
printf 'old_audio=%s\\nold_backend=%s\\ncandidate_audio=%s\\n' "$old_audio_image" "$old_backend_image" "$candidate_audio_image" | tee "$EVIDENCE_DIR/promotion-digests.txt"
test "$old_audio_image" = bmo-audio@sha256:62ad9adead83d863ab2bf28a2ac75e5a116dc68bab8ff06eec81b7a0407ddb34
...
```bash
install -m 600 "$EVIDENCE_DIR/rollback/audio.env" /opt/bmo/config/audio.env.rollback-copy
install -m 600 /opt/bmo/config/piper-only-candidate.env /opt/bmo/config/audio.env.piper-only
mv -f /opt/bmo/config/audio.env.piper-only /opt/bmo/config/audio.env
__omp_shell("rg -n -i 'kokoro|rvc|fallback' /opt/bmo/config/audio.env")
sha256sum /opt/bmo/config/audio.env | tee "$EVIDENCE_DIR/audio-env-piper-only.sha256"
```
...
BACKEND_ENV_FILE=/opt/bmo/config/p9.1/backend.env \
AUDIO_ENV_FILE=/opt/bmo/config/audio.env \
docker compose --project-name bmo-production -f /opt/bmo/app/docker-compose.yml up -d --no-build --pull never --no-deps audio
```

The command must recreate only `bmo-production-audio-1`. Confirm Backend, PostgreSQL, Hermes, Caddy, WhatsApp, and monitoring container identities are unchanged. Do not use `docker compose down`.
...
docker inspect bmo-production-audio-1 --format 'image={{.Config.Image}} id={{.Image}} status={{.State.Status}} health={{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}} restarts={{.RestartCount}} oom={{.State.OOMKilled}}' | tee "$EVIDENCE_DIR/production-audio-after-promote.txt"
```
...
```bash
PROD_TOKEN_FILE=$(mktemp /dev/shm/bmo-audio-token.XXXXXX)
chmod 600 "$PROD_TOKEN_FILE"
awk -F= '$1 == "INTERNAL_SERVICE_TOKEN" { sub(/^[^=]*=/, ""); print; exit }' /opt/bmo/config/audio.env > "$PROD_TOKEN_FILE"
test -s "$PROD_TOKEN_FILE"
cd /opt/bmo/app
python3 audio-service/scripts/verify_voice_pipeline.py \
  --base-url http://127.0.0.1:8001 \
  --token-file "$PROD_TOKEN_FILE" \
  --wav /opt/bmo/temp/p8-piper-production/public-speech.wav \
...
```bash
DEVICE_ID=$(awk -F= '$1 == "DEVICE_ID" { sub(/^[^=]*=/, ""); print; exit }' /opt/bmo/config/p9.1/backend.env)
DEVICE_TOKEN=$(awk -F= '$1 == "DEVICE_TOKEN" { sub(/^[^=]*=/, ""); print; exit }' /opt/bmo/config/p9.1/backend.env)
test -n "$DEVICE_ID" -a -n "$DEVICE_TOKEN"
cd /opt/bmo/app
BMO_BASE_URL=http://127.0.0.1:3000 \
DEVICE_ID="$DEVICE_ID" \
DEVICE_TOKEN="$DEVICE_TOKEN" \
FAKE_ESP32_WAV_PATH=/opt/bmo/temp/p8-piper-production/public-speech.wav \
...
```bash
find /opt/bmo/models /opt/bmo/cache/audio /opt/bmo/temp /opt/bmo/archive -xdev \( -iname '*kokoro*' -o -iname '*rvc*' -o -iname '*rmvpe*' -o -iname '*hubert*' \) -print | sort | tee "$EVIDENCE_DIR/legacy-artifacts-before.txt"
du -sb /opt/bmo/models/runtime/kokoro-82m-af-heart /opt/bmo/models/kokoro /opt/bmo/archive/p8-rvc 2>/dev/null | tee "$EVIDENCE_DIR/legacy-artifact-sizes.txt"
```

Expected current targets include the Kokoro runtime model, Kokoro model directory, and the exact Kokoro HuggingFace snapshot/lock paths. Keep `/opt/bmo/archive/p8-rvc` as historical evidence unless a separate retention decision explicitly authorizes archiving it elsewhere. Shared `hf-cache` roots, Whisper, Piper, PostgreSQL, and current production rollback image are not deletion targets.

- [ ] **Step 2: Delete only explicit obsolete paths after verification**

Use explicit reviewed paths, not a wildcard over model/cache roots. Keep `/opt/bmo/archive/p8-rvc` untouched as historical evidence. The deletion command must be executed only after Tasks 5 and 6 pass and the old image is retained:

```bash
rm -rf -- /opt/bmo/models/runtime/kokoro-82m-af-heart /opt/bmo/models/kokoro /opt/bmo/models/hf-cache/hub/models--hexgrad--Kokoro-82M /opt/bmo/models/hf-cache/hub/.locks/models--hexgrad--Kokoro-82M
```

Before running it, confirm the exact path list from `legacy-artifacts-before.txt`; if any path differs or contains shared content, stop and revise the target list. Do not delete the whole `/opt/bmo/models/runtime`, `/opt/bmo/models/hf-cache`, `/opt/bmo/cache/audio`, `/opt/bmo/temp`, or `/opt/bmo/archive` directory.
...
```bash
__omp_shell("rg -n -i 'kokoro|rvc|fallback' /opt/bmo/config/audio.env")
rg -n '^(PIPER|WHISPER|MODEL|AUDIO_SERVICE|INTERNAL_SERVICE_TOKEN|HF_|TRANSFORMERS_|ORT_|TTS_TEMP_DIR)' /opt/bmo/config/audio.env | cut -d= -f1 | sort
```
...
```bash
cd /opt/bmo/app
rg -n -i --hidden --glob '!.git/**' --glob '!node_modules/**' --glob '!**/__pycache__/**' --glob '!**/.pytest_cache/**' --glob '!docs/archive/**' --glob '!docs/backend-mvp/P8-*.md' 'kokoro|KOKORO_|rvc|RVC_|use_rvc|rvc_available|rvc_applied|rvcApplied|X-RVC-Applied|kokoro_loaded|TTS_FALLBACK_ENGINE' audio-service backend ops tests /opt/bmo/config/audio.env
```
...
```bash
docker inspect bmo-production-audio-1 bmo-production-p9-backend-1 bmo-production-p9-postgres-1 --format '{{.Name}} image={{.Image}} status={{.State.Status}} health={{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}} restarts={{.RestartCount}} oom={{.State.OOMKilled}}'
curl --fail --silent --show-error http://127.0.0.1:8001/readyz
curl --fail --silent --show-error http://127.0.0.1:3000/readyz
```
...
```text
BMO_PIPER_ONLY_PURGE_RESULT=PASS
```
...
```text
BMO_PIPER_ONLY_PURGE_RESULT=BLOCKED
```
