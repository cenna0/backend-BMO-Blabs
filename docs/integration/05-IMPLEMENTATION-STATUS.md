...
|---|---|---|
| Spotify concrete provider client | `SOURCE_READY` | Server-side Authorization Code exchange, required `/me` `account_id` identity with optional non-canonical profile `id` metadata, market-aware bounded search, normalized devices/playback, strict semantic URI/action mapping, and typed 401/403/429/5xx handling are implemented and mocked-tested. |
...
| Spotify encrypted persistence/refresh | `SOURCE_READY` | Dedicated `SPOTIFY_TOKEN_ENCRYPTION_KEY` AES-256-GCM storage, canonical `spotifyAccountId` plus non-canonical `spotifyProfileId`/market/authorization timestamp/preferred-device fields, account ownership uniqueness, keyed refresh single-flight plus database advisory lock, six-month lifecycle, replacement-token preservation, and invalid-grant wipe/reconnect behavior are implemented and mocked-tested. |
...
| WhatsApp candidate runtime/source alignment | `CANDIDATE_VERIFIED` | Candidate runs `bmo-phase26-whatsapp-candidate:789391b` from pushed SHA `789391b` (`789391b` full SHA in Git), image ID `sha256:ce601e2e72e1022890308982517bf626819d356e7efc66e771a110471462d30a`, and was recreated only for `bmo-p9-1-backend-1`; the identity-alias migration is applied only to candidate PostgreSQL. |
...
| Spotify adapter/catalog | `SOURCE_READY` | Concrete server-side provider adapter, `account_id`-canonical owner-scoped encrypted credentials, non-canonical profile metadata, semantic action boundary, deterministic resolver, device precedence, safe error projection, and candidate-only OAuth configuration are implemented and tested; live provider remains blocked |
