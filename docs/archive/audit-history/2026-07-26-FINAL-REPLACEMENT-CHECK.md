...
- Active PRD v1.2.1 still contained historical `/opt/joy-mvp`, public-IP staging examples, generic `api.<domain>`, old single-phase deployment assumptions, and stale sprint checklists.
...
- Synced PRD deployment context to `/opt/joy`, `main`, Caddy, `api.personaljoy.web.id`, `monitor.personaljoy.web.id`, Tailscale, Beszel/Telegram, backup/rollback, and P6–P10 ownership.
- Preserved Hardware Contract v1.0.5 unchanged.
- Preserved runtime target `medium` + hotword `Joy` and Kokoro `af_heart` speed `0.80`.
